...
public class AdminController {
	...
	@GetMapping("/guides")
	public ApiResponse<PageResponse<GuideResponse>> listGuides(
			@PageableDefault(size = 20, sort = "rating", direction = Sort.Direction.DESC) Pageable pageable) {
		return ApiResponse.ok(adminService.listGuides(pageable));
	}

	@PostMapping("/guides")
	public ApiResponse<GuideResponse> createGuide(@Valid @RequestBody CreateAdminGuideRequest request) {
		return ApiResponse.ok("Guide created", adminService.createGuide(request));
	}
}
